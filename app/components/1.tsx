  ...
  return (
    <div className="mx-auto max-w-screen-2xl px-4 md:px-8">
      <header className="mb-4 flex items-center justify-between py-4 md:py-8">
        <Link href="/" passHref> {/* Use Link from Next.js */}
          <a className="inline-flex items-center gap-2.5 text-xl font-bold text-black md:text-3xl" aria-label="logo">
            <TbAlignBoxRightBottomFilled className="text-2xl lg:text-4xl" />
            / Amir I.
          </a>
        </Link>

        <nav className={`gap-8 lg:flex ${isMobileMenuOpen ? 'block' : 'hidden'} lg:inline`}>
          <Link href="/" passHref> {/* Use Link from Next.js */}
            <a className="text-base font-semibold text-indigo-500 lg:text-lg">Home</a>
          </Link>
          <Link href="/about" passHref> {/* Use Link from Next.js */}
            <a className="text-base font-semibold text-indigo-500 lg:text-lg">About me</a>
          </Link>
        </nav>
       a
